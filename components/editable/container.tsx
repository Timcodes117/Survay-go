...
import {
  ElementCommentPin,
  ElementCommentPlacementComposer,
} from "./element-comment-anchor";
...
const EditableElementContainer = ({
  pageId,
  element,
  children,
  onChange,
}: EditableElementContainerProps) => {
  const {
    selectedElementId,
  ...
  const elementComments = element
    ? (elementCommentsByElementId[element.id] ?? [])
    : [];
  const hasElementComments = elementComments.length > 0;
  const showCommentComposer =
    !!element &&
    commentPlacementActive &&
    commentTargetElementId === element.id &&
    !hasElementComments;
  const showCommentPin = !!element && hasElementComments;

  const handleSelect = (e: React.MouseEvent<HTMLDivElement>) => {
    if (!element) return;
    e.stopPropagation();
    if (commentPlacementActive) {
      setCommentTargetElementId(element.id);
  ...
  if (!element) {
    return (
      <div
        ...
        className="w-full h-fit rounded-lg border border-dotted p-6 flex flex-col"
      >
        <EditableWrapper>
          <b
            className="text-md"
        ...
        >
          ...
          culpa molestias? Explicabo doloribus, molestiae vitae{" "}
        </p>
        {children}
      </div>
    );
  ...
  if (
    ...
    return (
      <div
        ...
        className={`relative overflow-visible scroll-mt-20 w-full h-fit rounded-md transition-colors ${isSelected ? "ring-2 ring-primary/60 bg-primary/5" : "hover:bg-muted/20"}`}
      >
        {showCommentPin ? <ElementCommentPin elementId={element.id} /> : null}
        {showCommentComposer ? (
          <ElementCommentPlacementComposer
  ...
  return (
    <div
      ...
      data-element-id={element.id}
      onClick={handleSelect}
      className={`relative overflow-visible scroll-mt-20 w-full h-fit rounded-lg border border-dotted p-6 flex flex-col transition-colors ${isSelected ? "ring-2 ring-primary/60 bg-primary/5" : "hover:bg-muted/20"}`}
    >
      {showCommentPin ? <ElementCommentPin elementId={element.id} /> : null}
      {showCommentComposer ? (
        <ElementCommentPlacementComposer
      ...
      <div className="flex items-center gap-2 text-xs my-3">
        <Input
          ...
          onMouseDown={(e: React.MouseEvent) => e.stopPropagation()}
        />{" "}
        <label id="required-label" htmlFor={`required-checkbox-${element.id}`}>
          Required
        </label>
      </div>
    </div>
